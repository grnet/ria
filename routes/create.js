...
var upload = multer({ storage: storage }).fields([{ name: 'field_21_upload', maxCount: 10 }, { name: 'field_23_upload', maxCount: 10 }, { name: 'field_36_upload', maxCount: 10 }, { name: 'nomosxedio', maxCount:1 }]);
...
routes.post('/:analysis', authUser, upload, [check('title', 'Title is required').notEmpty()], async function (req, res, next) {
    ...
    try {
        ...
        if (file.nomosxedio) {
            nomosxedio.push({filename:file.nomosxedio[0].filename, upload_date:req.body.initial_submit})
        }        
    } catch (e) {
    ...
    await database.ekthesi_tables.create({ static_tables: field_9, checkbox_tables: checkbox_tables, ekthesi_tablesId: res_data.id });
    ...
    await database.ekthesi.update({
        ...
        field_21_upload: field21, field_23_upload: field23, field_36_upload: field36, nomosxedio: nomosxedio
    },
        {
            where: {
                id: res_data.id
